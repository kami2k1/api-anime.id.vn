...
class UploadFrame(ctk.CTkFrame):
    def __init__(self, parent, api):
        ...
        self.grid_rowconfigure(2, weight=1)  # Progress area can expand
        ...
        form.grid(row=1, column=0, sticky="ew", padx=20, pady=20)
        ...
        self.status = StringVar(value="Ready to upload")
        self.progress_text = StringVar(value="")
        
        # File selection
        ctk.CTkLabel(form, text="Video File:").grid(row=0, column=0, sticky="w", pady=10, padx=5)
        
        file_frame = ctk.CTkFrame(form, fg_color="transparent")
        file_frame.grid(row=0, column=1, sticky="ew", pady=10)
        ...
        # Title
        ctk.CTkLabel(form, text="Title:").grid(row=1, column=0, sticky="w", pady=10, padx=5)
        ctk.CTkEntry(form, textvariable=self.title).grid(row=1, column=1, sticky="ew", pady=10)
        
        # Lower resolutions option
        ctk.CTkCheckBox(
            form, 
            text="Create lower resolution versions", 
        ...
        ctk.CTkCheckBox(
            form, 
            text="Use GPU encoding (if available)", 
        ...
        progress.grid(row=2, column=0, sticky="nsew", padx=20, pady=20)
        progress.grid_columnconfigure(0, weight=1)
        
        ctk.CTkLabel(progress, textvariable=self.status, font=("Arial", 14, "bold")).grid(row=0, column=0, sticky="ew", pady=5)
        
        self.log_text = ctk.CTkTextbox(progress, height=200, wrap="word")
        self.log_text.grid(row=1, column=0, sticky="nsew", pady=10)
        
        self.progress_bar = ctk.CTkProgressBar(progress)
        self.progress_bar.grid(row=2, column=0, sticky="ew", pady=10)
        self.progress_bar.set(0)
        
        ctk.CTkLabel(progress, textvariable=self.progress_text).grid(row=3, column=0, sticky="ew", pady=5)
    ...
    def check_video_info(self):
        ...
        try:
            ...
            self.log(f"Error analyzing video: {str(e)}")
    ...
    def _analyze_video_threaded(self, file_path):
        try:
            ...
            self.log(f"Video information:")
            self.log(f"• Resolution: {width}x{height}")
            self.log(f"• Duration: {int(duration // 60)}m {int(duration % 60)}s")
            ...
            if height not in STANDARD_RESOLUTIONS:
                ...
                self.log(f"Note: Video height ({height}p) will be adjusted to standard resolution ({target_res}p)")
        except Exception as e:
            self.log(f"Error analyzing video: {str(e)}")
    ...
    def _check_gpu_threaded(self):
        ...
        if gpu_encoder == "h264":
            ...
            self.log("GPU encoding not available. Using CPU encoding.")
        else:
            self.log(f"Using GPU encoder: {gpu_encoder}")
    ...
    def clear_form(self):
        ...
        self.status.set("Ready to upload")
        self.log("Form cleared. Ready for a new upload.")
    ...
    def start_upload(self):
        ...
        if not file_path or not os.path.exists(file_path):
            self.log("Error: Please select a valid video file")
            return
            
        if not title.strip():
            self.log("Error: Please enter a title for the video")
            return
            
        # Disable upload button during processing
        self.upload_button.configure(state="disabled")
        self.status.set("Processing video...")
    ...
    async def _process_upload(self):
        ...
        try:
            ...
            self.update_progress(0.05, "Preparing for processing...")
            
            # Create a unique output folder
            output_folder = os.path.join("data", str(uuid.uuid4()))
            os.makedirs(output_folder, exist_ok=True)
            
            self.log(f"Processing in temporary folder: {output_folder}")
            
            # Get video info
            self.update_progress(0.1, "Analyzing video...")
            width, height = await self._run_async(self._get_video_info, file_path)
            
            # Adjust resolution if needed
            STANDARD_RESOLUTIONS = [2160, 1440, 1080, 720, 480, 360, 240, 144]
            if height not in STANDARD_RESOLUTIONS:
                self.log("Video height does not match standard resolutions.")
                oid = height
                for res in STANDARD_RESOLUTIONS:
                    if height < res:
                        ...
                        height = oid
                        break
                self.log(f"Adjusted resolution to: {height}p")
            
            # Create thumbnails
            self.update_progress(0.2, "Creating thumbnails...")
            ...
            # Create HLS streams
            self.update_progress(0.3, "Creating HLS streams...")
            self.log(f"Creating HLS stream for resolution: {height}p")
            if available_res:
                self.log(f"Creating additional streams for resolutions: {', '.join(str(r) + 'p' for r in available_res)}")
                
            await self._create_hls(file_path, available_res, height, output_folder, gpu_encoder)
            
            # Create master playlist
            self.update_progress(0.6, "Creating master playlist...")
            self._create_master_playlist(available_res, height, output_folder)
            
            # Upload files
            self.update_progress(0.7, "Uploading files...")
            ...
            # Upload thumbnails
            self.log("Uploading thumbnails...")
            thum = await api.upload(output_folder, ".jpg", 2, type=4)
            
            # Upload TS segments
            self.update_progress(0.8, "Uploading video segments...")
            tsfile = await api.upload(output_folder, ".ts", 0, type=4)
            
            # Upload manifest
            self.update_progress(0.9, "Uploading manifest...")
            data = {
                "id": tsfile,
                "tile": title,
                "time": duration,
            ...
            # Complete
            self.update_progress(1.0, "Upload complete!")
            self.status.set("Upload successful!")
            
            # Display results
            if result and result.get('code') == 0:
                if 'data' in result and result['data'].get('code') == 0:
                    self.log("Upload completed successfully!")
                    self.log(f"Link 1: {result['data'].get('link', '')}")
                    self.log(f"Link 2: {result['data'].get('link2', '')}")
                else:
                    self.log(f"Upload completed with status: {result}")
            else:
                self.log(f"Upload result: {result}")
    ...
    def _create_thumbnails(self, input_file, output_folder, sizes=[1080, 120, 180, 240, 360, 480, 60, 720], num_thumbs=1):
        """Create thumbnails synchronously"""
        os.makedirs(output_folder, exist_ok=True)
        try:
            probe = ffmpeg.probe(input_file)
            duration = float(probe["format"]["duration"])
    ...
    async def _create_hls(self, input_file, resolutions, original_resolution, output_folder, encoder):
        """Create HLS streams asynchronously"""
        os.makedirs(output_folder, exist_ok=True)
        tasks = []
        
        # Set the appropriate hardware acceleration flag based on encoder
        ...
        for res in [original_resolution] + resolutions:
            ...
            command = [
                ...
                "-fflags", "+bitexact",  # Don't include FFmpeg info
                "-f", "hls", output_file
            ]
            
            # Update UI before starting the process
            self.log(f"Creating HLS stream for {res}p resolution")
    ...
    def _create_master_playlist(self, resolutions, original_resolution, output_folder):
        ...
        self.log(f"Created master playlist with {len(resolutions) + 1} streams")
